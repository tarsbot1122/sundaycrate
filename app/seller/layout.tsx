  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      {user && !user.stripe_account_id && (
        <div className="mx-4 mt-4 rounded-lg bg-gold/20 border border-gold/30 p-3">
          <div className="flex items-start gap-2">
            <AlertTriangle className="h-4 w-4 text-gold mt-0.5 flex-shrink-0" />
            <div>
              <p className="text-xs font-semibold text-gold">Connect Stripe</p>
              <p className="text-xs text-white/70 mt-0.5 mb-2">
                Set up payouts to start selling
              </p>
              <Link
                href="/api/seller/connect"
                className="inline-flex items-center gap-1 text-xs font-semibold text-gold hover:text-white transition-colors"
                onClick={() => setSidebarOpen(false)}
              >
